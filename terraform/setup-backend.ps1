...
try {
    aws s3api create-bucket --bucket $BUCKET_NAME --region $REGION 2>$null
} catch {
    Write-Host "Bucket already exists or error occurred"
}
...
Write-Host "Enabling encryption on S3 bucket..."
...
try {
    ...
    Write-Host "Table already exists or error occurred"
}

Write-Host ""
Write-Host "✅ Backend setup complete!"
Write-Host "Now you can run 'terraform init' to migrate your state to S3"
